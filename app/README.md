...
録画・会議管理・接続設定を扱う Swift アプリ。製品コードは 5 つのターゲットからなる。この README は 0.6.2 の録画画面選択・統合 MP4 の表示と再生、および OpenAI 音声認識・6プロバイダの単独テキスト議事録生成を扱う。配布の公開状況は [GitHub Releases](https://github.com/btajp/narumi/releases) で別に確認する。
...
録画ファイルをコピー・共有するには、会議一覧から会議を選び、会議名の下にある「録画ファイル（MP4）」の「MP4 を Finder で表示」を押す。統合済みの `playback/recording.mp4` が直接選択される。「録画を再生」は既定の動画アプリで開く。「会議フォルダを Finder で表示」は元データや議事録を含むフォルダ全体を表示する操作で、録画ファイルを探すために使う必要はない。

0.6.2 から MP4 生成時にマイクとシステム音声の音量差を補正する（FFmpeg 5.1 以降が必要）。一時的な大音量はリミッターで抑える。旧版で生成した MP4 は「録画ファイルを再生成」で補正できる。元トラックは保持し、音声認識や議事録生成は実行しない。
...
- 要件: macOS 15 以降、Swift 6.0 以降のツールチェーン（6.3.3 で検証）。外部パッケージ依存は Sparkle（`NarumiMenuBar` のみが依存。初回ビルド時に GitHub から取得）だけで、引数解析は自前。
...
- 接続先の既定は `https://127.0.0.1:<NARUMI_SERVER_PORT または 8765>/mcp`。`NARUMI_SERVER_URL` を指定する場合も数値 loopback の HTTPS に限り、所有者専用の起動情報と一致する必要がある。証明書の pin・有効期限・接続先を検証してから Keychain の Bearer トークンで認証し、環境プロキシやリダイレクトを使わない。`MCPClient` は `initialize`（protocolVersion `2025-06-18`）→ `notifications/initialized` → `tools/call` を JSON-RPC 2.0 の POST で行い、`Mcp-Session-Id` を保持する。応答は `application/json` と `text/event-stream` を受け付ける。対応する契約版と認証付き TLS のメタデータを確認できなければ操作を開始しない。同梱サーバーは契約 6.0.0 と厳密に照合する。開発用の外部接続だけは旧契約 2 / 3 / 4 / 5 を各版の対応範囲で許可する。6プロバイダ選択には契約 6 と明示的な対応通知を要求し、旧版から対応を推測しない。製品 CLI は旧版互換を持たず、契約メジャー版 6 を要求する。
...
- **AI 接続シート**: Codex App Server / Claude Agent SDK / OpenAI API / OpenAI互換API / Anthropic API / Ollama の接続追加・編集・有効化・無効化・削除、認証・ログアウト、実行環境の準備、モデル候補の取得・明示検証。既存の9ツールに `verify_provider_model` を加え、認証状態・モデル取得状態・生成状態を別々に表示する。
...
実行環境の準備は `provider_setup` ジョブで追跡し、取消と状態確認を使う。配布版の Codex は `.app` に固定同梱した公式 CLI **0.150.1** を narumi 専用 runtime へコピーし、版・サイズ・SHA256 をオフラインで検証する。同梱物が欠落・変更していれば外部インストールへ切り替えず、更新または再インストールが必要な状態として表示する。Claude Agent SDK は固定版のSDK・CLI・narumi adapterを検証済みスナップショットから隔離プロセスで実行し、tools・MCP・履歴・自動再試行を無効にする。OpenAI / OpenAI互換 / Anthropic / Ollama は内蔵 HTTP アダプタの版・実行ソースを確認する。グローバルツールの追加、SDK・モデルのダウンロード、利用者指定コマンドの実行は提供しない。

候補一覧は画面表示時にはキャッシュだけを読み、明示的な更新で通信する。議事録は `role=llm`、音声認識は `role=transcription` としてカタログと UI キャッシュを分け、同じ OpenAI 接続でも候補を混ぜない。Claude Agent SDKとOpenAI互換APIは、候補取得後に固定の非秘密テスト文を明示確認付きで1回送信し、成功したモデルだけを選択可能にする。全工程のモデル選択、API の画像入力、複数案の生成・統合は未対応。接続確認・候補取得の成功を、実認識・実生成の成功とは扱わない。
...
`TranscriptionModelSelection` / `TranscriptionModelForm` / `TranscriptionModelCatalogStore` を議事録用とは別に持つ。「文字起こしの処理方法」は「ローカルの設定を使う」または「OpenAI API を使う」。後者では「音声認識用の OpenAI 接続」「音声認識モデル」を選び、「音声認識の候補を取得・更新」で明示的に照会する。候補は `whisper-1` / `gpt-4o-transcribe-diarize` に限り、時刻付き能力・接続版・認証・実行環境・提供終了日を検証する。契約6の常駐サーバーが `transcription_model_providers` で通知した経路だけを提供し、通知は認証成功とは扱わない。
...
契約 4 / 5 / 6 の `minutes_model_providers` はサーバーが対応する生成経路を示し、接続が認証済みか・モデルを利用できるかとは別に扱う。`stage_model_selection` が有効な場合に、通知された対応系統だけを選択肢にする。旧契約 3 では Codex に限定し、2 ではこの選択を提供しない。

「推論量」は Codex / OpenAI の対応を確認したモデルだけに表示し、空欄は「モデルの既定値」として解決する。「出力上限（トークン）」は OpenAI / OpenAI互換 / Anthropic / Ollama で 1〜32,768 の整数を受け付ける。空欄は 4,096 と既知モデル上限の小さい方、不明ならアプリ既定の 4,096。モデルの `max_output_tokens` が null なら「未確認」のまま表示する。要求上限は 1 回あたりの制限で、分割生成の総利用量や金額を保証しない。Codex / Claude Agent SDK の出力トークン上限は指定できない。
...
`minutes_model` の省略は保持、null は解除、オブジェクトは全体置換。「従来設定」を選んで保存すると議事録用の個別選択を解除する。`llm_provider` は発話統合・Vision など従来の工程に残し、議事録モデルを共用しない。`local_only` は自動変更しない。Claude Agent SDK / OpenAI / OpenAI互換 / Anthropic は `api_ok`、Codex は `subscription_ok` / `api_ok` が必要で、Ollama はローカル確認済みなら `local_only` のままでよい。Codex はどちらの許可でも ChatGPT の利用枠を使い、API キー方式へ切り替えない。
...
接続を指定した 6 プロバイダすべてで `force=true` を禁止し、同じ入力・選択・実行環境では保存済み結果を再利用する。送信後の結果が不明なら、narumi は生成を自動で開始し直さない。「新しく生成を試す…」で API 利用料や ChatGPT 利用枠の重複消費を確認し、「試行番号を増やす（保存後に再生成）」で `cache_epoch` を増やす。この操作だけでは保存・送信せず、保存後に再生成する。Ollama はローカル処理を再実行する。モデルだけの変更で文字起こし・発話統合をやり直さず、過去の議事録版を保持する。

生成入力は 6 プロバイダともテキストだけとし、音声・動画・画像を渡さない。HTTP系は proxy・redirect・自動再送を禁止し、互換APIは接続時に固定したsurfaceから切り替えない。Claude Agent SDKはtools・MCP・履歴・自動再試行を無効にする。同期応答の取消は接続を切るだけで、サービス側の処理や課金停止を保証しない。API の利用量が欠けた場合は不明とし、ゼロや無料と推測しない。OpenAI の `store=false` はサービス側の保持ゼロを意味しない。Ollama の Modelfile に組み込まれた system / messages は継承される場合があり、narumi のプロンプトだけで生成されることは保証しない。
...
5. **起動待ち**: 1 秒ごとに認証付き接続で `get_server_info` を最大 30 秒試し、サーバー識別情報・対応する契約版・TLS 必須のメタデータを確認したら「稼働中 v… 契約 …」。bundled モードは版・契約 6.0.0・実体パスも厳密に照合する。失敗時は自分が起動したサーバーを停止し、未確定の runtime 切替を復元する。repo モードは、タイムアウト時も生きているプロセスを残し、終了または再起動で停止する。起動直後にプロセスが終了した場合も「起動失敗（ログ参照）」、稼働後に終了すると「停止 (exit N)」。
