...
- 会議詳細の「録画ファイル（MP4）」に「MP4 を Finder で表示」を追加。統合済みの recording.mp4 を直接選択表示し、フォルダを探さずにコピー・共有できるように
- 再生ボタンを「録画を再生」に変更し、会議フォルダを開く操作と統合ファイルを表示する操作を明確化
- マイクとシステム音声のラウドネスを個別に測定し、音量差を補正してから MP4 に統合。増幅量に上限を設け、一時的な大音量はリミッターで抑え、無音や微小音は増幅しない
- 既存録画は「録画ファイルを再生成」で音量補正を適用できる。元トラックと音声の開始時刻・長いトラックの末尾は保持
- 契約は 6.1.0・40 ツールのまま。MP4 の保存場所は playback/recording.mp4。音量補正には FFmpeg 5.1 以降が必要
- 新しい Finder 導線と音量補正の実機確認は、公開後の Sparkle 更新で実施予定

## 0.6.1 - 2026-09-08

- 録画対象がディスプレイ一覧の先頭になる不具合を修正。メイン画面を既定にし、開始前に画面名と解像度を見て選択可能に
- 停止後に映像・マイク・システム音声をまとめた再生用 MP4 をローカル生成。話者分離用の元トラックを保持し、音声の開始時刻と長いトラックの末尾を維持
- 会議詳細に録画の生成・再生を追加。過去の録画も保存済みトラックから生成でき、文字起こしや外部送信は実行しない
- 録画プロセスが途中で終了しても録画中表示が続く問題を修正。終了を検知するとアプリが保存へ進み、画面取得エラーを停止結果と会議詳細に表示
- 元トラックの破棄時には、その内容を含む再生用 MP4 も削除。生成の失敗・取消時は元トラックを保持
- 契約を 6.1.0・40ツールへ更新。全ディスプレイの同時録画と、録画されなかった画面の復元は対象外
- 複数ディスプレイでの実録画、画面切断時の実機動作、公開後の Sparkle 更新とアプリ操作は公開前時点では未検証
...
- mlx-whisper に正規化済み WAV データを直接渡し、GUI アプリの最小 `PATH` で内部の `ffmpeg` を起動できず文字起こしが失敗する問題を修正
- TLS 接続中のキャンセルを最大 0.1 秒間隔で検出し、通信処理が期限まで残り続ける問題を修正
