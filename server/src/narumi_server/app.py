...
def dispatch(ctx: ServerContext, tool: str, arguments: Mapping[str, Any] | None) -> ToolOutcome:
    ...
    try:
        ctx.contracts.validate_input(tool, args)
        ...
        result = ctx.idempotency.run(contract, args, lambda: handler(ctx, args))
    ...
    return ToolOutcome(payload=_checked_envelope(ctx, envelope, tool), is_error=True)


def _checked_envelope(ctx: ServerContext, envelope: dict[str, Any], tool: str) -> dict[str, Any]:
