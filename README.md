...
**0.6.1** では録画対象の選択と、分離トラックからの再生用 MP4 生成・再生に対応しました。録画は選択した1画面を対象とし、既定はメイン画面です。
...
5. 録画中は同じ画面に経過時間と「録画停止」が表示されます。終了後は会議一覧から結果を開けます。会議詳細の統合録画欄から生成済み MP4 を再生でき、過去の録画は「録画ファイルを生成」で準備します。元の映像・マイク・システム音声は別トラックのまま保持します。
