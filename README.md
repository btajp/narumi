...
**0.6.2** では、会議詳細の「録画ファイル（MP4）」から統合ファイルを Finder で直接表示できるようになりました。マイクとシステム音声の音量差も、個別にラウドネスを測定して補正します。録画は選択した1画面を対象とし、既定はメイン画面です。

0.6.0 から、AI 接続を Codex App Server / Claude Agent SDK / OpenAI API / OpenAI互換API / Anthropic API / Ollama の順に統一し、全 6 種で接続・モデル選択・単一プロバイダによるテキスト議事録生成に対応しました。既定のローカル Whisper と OpenAI API 音声認識は工程別に選択できます。全工程のモデル選択、API の画像入力、複数プロバイダによる生成・統合は後続です。
...
4. 両方が許可済みになったら「録画開始」を押し、会議名を入力して録画対象の画面を選びます。既定はメイン画面です。プロファイルと scope は空欄なら既定設定を使います。macOS がアプリの再起動を求めた場合は、その案内に従ってから再試行します。
5. 録画中は同じ画面に経過時間と「録画停止」が表示されます。終了後は会議一覧から結果を開けます。会議詳細の「録画ファイル（MP4）」で「MP4 を Finder で表示」を押すと、画面・マイク・システム音声をまとめた MP4 が選択され、そのままコピー・共有できます。「録画を再生」でも開けます。過去の録画は「録画ファイルを生成」で準備します。
...
| 契約（`contracts/`）と MCP サーバー（契約 6.1.0・40 ツール） | 実装・ソース検証済み。6 プロバイダと明示的なモデル検証を公開。常駐は認証付き TLS、外部 MCP クライアント向け stdio bridge、開発用 stdio |
