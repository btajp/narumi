...
**0.4.0** では、議事録用の接続・モデル選択を Codex App Server / OpenAI API / Anthropic API / ローカル Ollama に広げました。実装・ソース検証済みです。選んだ一つのモデルでテキスト議事録を生成し、文字起こし・発話統合・画像読解の設定は維持します。OpenAI 音声認識、Claude Agent SDK による生成、複数案の生成と統合は後続です。
...
| 契約（`contracts/`）と MCP サーバー（契約 4.0.0・37 ツール） | 実装・ソース検証済み（常駐は認証付き TLS、外部 MCP クライアント向け stdio bridge、開発用 stdio） |
...
新しい接続・モデル設定を適用するのは、**4 系統による単独のテキスト議事録生成**です。全工程のモデル選択、OpenAI 音声認識、複数プロバイダでの生成・統合は未対応です。Claude Agent SDK は API キー方式のみを扱い、認証・履歴の隔離を確認できるまで生成を停止しています。接続テストの成功やモデル候補の表示は、実際の議事録生成の成功を意味しません。今回の範囲は [API・ローカル議事録生成の設計](docs/superpowers/specs/2026-08-29-openai-minutes-design.md)、Codex の認証・実行境界は [Codex 議事録生成の設計](docs/superpowers/specs/2026-08-29-codex-minutes-design.md)、後続の範囲は [プロバイダ設定の設計](docs/superpowers/specs/2026-08-28-provider-workflow-design.md) と [実装計画](docs/superpowers/plans/2026-08-28-provider-workflow.md) を参照してください。

0.4.0 のソース検証結果（2026-08-29）は次のとおりです。通常のテストでは fake とローカル TLS を使います。過去の 0.3.0 の検証結果は [実装計画の検証記録](docs/superpowers/plans/2026-08-28-provider-workflow.md) に保持しています。
...
上記は偽の API キーを使うソース・プロトコル検証です。実 ChatGPT ログイン・実生成、実 API・実 Keychain、アプリ更新後の画面操作は未検証であり、署名・公証・配布物照合や公開成功を示すものではありません。
...
| OpenAI API | API キー。`https://api.openai.com` 固定。ChatGPT の利用枠とは別の API 課金 | 接続・モデル選択・テキスト議事録生成 |
...
4 系統の議事録モデル選択には契約 4.x が必要です。配布アプリは同梱サーバーの契約 4.0.0 と厳密に照合し、製品 CLI は契約メジャー版 4 を要求します。開発用に外部サーバーへ接続する Swift アプリだけは、旧契約 2 / 3 もその版の対応範囲で許可します。通常はアプリ・サーバー・CLI を同じ対応版に揃え、未認証 HTTP へダウングレードしません。
